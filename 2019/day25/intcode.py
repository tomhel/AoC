#!/usr/bin/env python3


import queue


class TerminalIOHandler(object):

    def __init__(self):
        pass

    def send_input(self, i):
        raise NotImplementedError()

    def read(self):
        return int(input(">>> "))

    def write(self, v):
        print(v)

    def dump_output(self):
        return []

    def get_output(self):
        return None

    def exit(self):
        pass

    def interrupt(self):
        pass


class BufferIOHandler(object):

    def __init__(self, input_values=None):
        self.input_values = input_values[:] if input_values is not None else []
        self.in_buffer = iter(self.input_values)
        self.output_values = []
        self.out_buffer = iter(self.output_values)

    def send_input(self, i):
        self.input_values.append(i)

    def read(self):
        return next(self.in_buffer)

    def write(self, v):
        self.output_values.append(v)

    def dump_output(self):
        return list(self.out_buffer)

    def get_output(self):
        return next(self.out_buffer, None)

    def exit(self):
        pass

    def interrupt(self):
        pass


class PipeIOHandler(object):

    def __init__(self, in_pipe, out_pipe, input_values=None):
        self.in_pipe = in_pipe
        self.out_pipe = out_pipe
        if input_values is not None:
            for v in input_values:
                self.in_pipe.put(v)

    def send_input(self, i):
        self.in_pipe.put(i)

    def read(self):
        v = self.in_pipe.get()
        self.in_pipe.task_done()
        return v

    def write(self, v):
        self.out_pipe.put(v)

    def dump_output(self):
        dump = []

        while True:
            try:
                v = self.out_pipe.get_nowait()
                self.out_pipe.task_done()
                if v is not None:
                    dump.append(v)
            except queue.Empty:
                break

        return dump

    def get_output(self):
        v = self.out_pipe.get()
        self.out_pipe.task_done()
        return v

    def exit(self):
        self.out_pipe.put(None)

    def interrupt(self):
        self.in_pipe.put(None)


def load_program(path):
    with open(path) as f:
        return [int(i) for i in f.read().split(",")]


class Computer(object):

    def __init__(self, program, io_handler):
        self.pc = 0
        self.rel_base = 0
        self.mem = {adr: i for adr, i in enumerate(program)}
        self.io_handler = io_handler
        self.suspended = False

    def _read_value(self, adr, mode):
        if mode == 0:  # position
            return self.mem.get(self.mem.get(adr, 0), 0)
        elif mode == 1:  # immediate
            return self.mem.get(adr, 0)
        elif mode == 2:  # relative
            return self.mem.get(self.mem.get(adr, 0) + self.rel_base, 0)
        else:
            raise Exception("unsupported mode for reads: %d" % mode)

    def _write_value(self, adr, mode, value):
        if mode == 0:  # position
            self.mem[self.mem.get(adr, 0)] = value
        elif mode == 2:  # relative
            self.mem[self.mem.get(adr, 0) + self.rel_base] = value
        else:
            raise Exception("unsupported mode for writes: %d" % mode)

    def input_int(self, i):
        self.io_handler.send_input(i)

    def input_text(self, text):
        for c in text:
            self.io_handler.send_input(ord(c))

    def suspend(self):
        self.suspended = True
        self.io_handler.interrupt()

    def execute(self):
        self.suspended = False

        while not self.suspended:
            op = int(str(self.mem[self.pc])[-2:])
            modes = [int(m) for m in str(self.mem[self.pc])[:-2].rjust(3, "0")]

            if op == 1:  # add
                v1 = self._read_value(self.pc + 1, modes[2])
                v2 = self._read_value(self.pc + 2, modes[1])
                self._write_value(self.pc + 3, modes[0], v1 + v2)
                self.pc += 4
            elif op == 2:  # multiply
                v1 = self._read_value(self.pc + 1, modes[2])
                v2 = self._read_value(self.pc + 2, modes[1])
                self._write_value(self.pc + 3, modes[0], v1 * v2)
                self.pc += 4
            elif op == 3:  # input
                v1 = self.io_handler.read()
                if v1 is None:
                    continue
                self._write_value(self.pc + 1, modes[2], v1)
                self.pc += 2
            elif op == 4:  # output
                v1 = self._read_value(self.pc + 1, modes[2])
                self.io_handler.write(v1)
                self.pc += 2
            elif op == 5:  # jump if not zero
                v1 = self._read_value(self.pc + 1, modes[2])
                self.pc = self._read_value(self.pc + 2, modes[1]) if v1 != 0 else self.pc + 3
            elif op == 6:  # jump if zero
                v1 = self._read_value(self.pc + 1, modes[2])
                self.pc = self._read_value(self.pc + 2, modes[1]) if v1 == 0 else self.pc + 3
            elif op == 7:  # less than
                v1 = self._read_value(self.pc + 1, modes[2])
                v2 = self._read_value(self.pc + 2, modes[1])
                self._write_value(self.pc + 3, modes[0], int(v1 < v2))
                self.pc += 4
            elif op == 8:  # equal
                v1 = self._read_value(self.pc + 1, modes[2])
                v2 = self._read_value(self.pc + 2, modes[1])
                self._write_value(self.pc + 3, modes[0], int(v1 == v2))
                self.pc += 4
            elif op == 9:  # adjust relative base
                v1 = self._read_value(self.pc + 1, modes[2])
                self.rel_base += v1
                self.pc += 2
            elif op == 99:  # quit
                break
            else:
                raise Exception("unknown op: %d" % op)

        self.io_handler.exit()

    def get_output(self):
        return self.io_handler.dump_output()

    def get_decoded_output(self):
        text = ""

        while True:
            i = self.io_handler.get_output()

            if i is None and len(text) == 0:
                return None
            elif i == 10:
                return text
            elif 0 <= i <= 127:
                text += chr(i)
            elif len(text) == 0:
                return i
            else:
                raise Exception("unable to decode: %s" % i)

    def print_output(self):
        for i in self.io_handler.dump_output():
            if 0 <= i <= 127:
                print(chr(i), end="")
            else:
                print(i)
