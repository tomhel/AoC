...
def largest_number_of_geodes():
    count = 1
    for blueprint in list(load())[:3]:
        inventory = {"ore": 0, "clay": 0, "obsidian": 0, "geode": 0}
        robots = ["ore"]
        geodes = get_maximum_geode_count(blueprint, inventory, robots, 0, set(), [0])
        count *= geodes
    return count


# takes about 2.5 minutes to run using PyPy.
print(largest_number_of_geodes())
